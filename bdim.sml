...
fun isValid(string_value) = let fun ifStatementValid(string_value,i) = if i>= size(string_value)-1 then true else (if isdigit(String.substring(string_value,i,1)) then ifStatementValid(string_value,i+1) else  raise SyntaxError) in ifStatementValid(string_value,0) end;
